...
#define LV_DRV_DELAY_US(us)   HAL_Delay(1);         /*delay_us(us)*/       /*Delay the given number of microseconds*/
#define LV_DRV_DELAY_MS(ms)   HAL_Delay(ms);        /*delay_ms(ms)*/       /*Delay the given number of milliseconds*/
...
#define LV_DRV_DISP_CMD_DATA(val)  HAL_GPIO_WritePin(LCD_DC_GPIO_Port, LCD_DC_Pin, val);;	/*pin_x_set(val)*/    /*Set the command/data pin to 'val'*/
#define LV_DRV_DISP_RST(val)       HAL_GPIO_WritePin(LCD_RESET_GPIO_Port, LCD_RESET_Pin, val);   /*pin_x_set(val)*/    /*Set the reset pin to 'val'*/

/*---------
 *  SPI
 *---------*/
#define LV_DRV_DISP_SPI_CS(val)          HAL_GPIO_WritePin(SPI2_CS_SW_GPIO_Port, SPI2_CS_SW_Pin, val);  /*spi_cs_set(val)*/     /*Set the SPI's Chip select to 'val'*/
#define LV_DRV_DISP_SPI_WR_BYTE(data)    HAL_SPI_Transmit(HSPI_INSTANCE, &data, 1, 1);     /*spi_wr(data)*/        /*Write a byte the SPI bus*/
#define LV_DRV_DISP_SPI_WR_ARRAY(adr, n) HAL_SPI_Transmit(HSPI_INSTANCE, adr, n, 1);       /*spi_wr_mem(adr, n)*/  /*Write 'n' bytes to SPI bus from 'adr'*/
